...
// 객체가 명시된 타입으로 형변환 될 수 있는가?

if(객체 instanceof 타입) { }  

if(v1 instanceOf Bus){ //v1이 Bus 타입으로 형변환이 가능하다면
  Bus bus = (Bus)v1; // v1을 Bus로 강제 타입변환(Bus 객체가 Vehicle로 형변환 됐었기에 강제 형변환 가능, 이런식으로 Bus객체인지 Texi 객체인지 판별 가능)
}
```

## 5. 인터페이스 상속
+ 인터페이스는 다른 인터페이스를 상속 가능(다중상속 가능)  
+ cf) 인터페이스는 클래스를 상속할 순 없음 - 인터페이스는 정적 파이널 필드와 추상메소드를 가지는데 일반 클래스를 상속받으면 동적 필드와 메소드들을 할당 받기 때문

```
// 상위 인터페이스 Human과 Dinorsore의 상속을 받는 하위 인터페이스 Animal 

public interface Animal  extends Human, Dinorsore{
  
}
```

### 5-1. 하위인터페이스의 구현 객체
+ **하위 인터페이스의 구현 객첻들은 상위인터페이스의 추상메소드들을 모두 구현해야함**
+ 구현 클래스로 부터 객체를 생성한 후에 하위 및 상위 인터페이스 타입으로 타입변환 가능(실체 메소드를 모두 가지고 있기 때문)

```
Animal a1 = new Cat(); //Cat 구현 클래스안에 하위인터페이스의 실체메소드가 존재하기 때문에 타입변환 가능
                      // Animal, Dinosore, Human 에 존재하는 모든 메소드 사용가능

Human h1 = new Cat(); //Cat 구현 클래스안에 상위 인터페이스의 실체메소드가 존재하기 때문에 타입변환 가능
                      // Human 인터페이스에 존재하는 메소드만 사용가능

Dinorsore d1 = new Cat(); //Cat 구현 클래스안에 상위 인터페이스의 실체메소드가 존재하기 때문에 타입변환 가능
                          // Dinorsore 인터페이스에 존재하는 메솓드만 사용가능
```

  

## 질문1. Q) 왜 인터페이스에는 생성자가 없고 추상클래스에는 생성자가 있을까

+ A) 인터페이스는 지정된 설계도와 같은 개념이어서 객체를 만들 수 없고 자식클래스를 만들 수 없다.
+  객체를 만들 수 없기 때문에 초기화에 필요한 생성자는 필요하지 않고, 자식클래스를 만들 수 없기 때문에 자식클래스에서 객체를 초기화할때
+  부모생성자를 호출해 초기화 할 필요도 없기 때문에 생성자가 있을 이유가 없다

+ A) 반면 추상클래스의 경우 추상적인 설계도 이기에 객체를 만들 수 없는것은 같지만 추상클래스는 지정된 설계도 라기 보다는
+  인터페이스 보다는 작은 개념의 설계도로 확장이 가능한 설계도와 같다 (ex. 계속 추가해 나가는 설계도)
+  때문에 자식클래스를 이용해 확장이 가능하며 자식클래스에서 상속받은 부모객체를 사용하기 위해서는 부모 객체의 초기화가 필요하기 때문에 부모 생성자를 이용해 초기화하면서 무조건 생성자가 필요해 지는것이다.

## 질문 2. Q) 인터페이스의 구현객체와 상속의 자식클래스의 차이점(static final은 상속이 불가능한데 구현객체에서 사용이 가능한 이유)
+ Q++) 구현객체가 상속과 다른점이 무엇이길래 static final을 사용할 수 있는가
+ 일반적인 클래스의 객체를 생성했을때 생성된 객체는 클래스의 모든 정보를 가지고 있다
+ 의 개념과 비슷하게 인터페이스의 구현 클래스는 인터페이스의 모든 정보를 다 가지고 있다는 것이다(다른 개념이지만 이해를 위해 가져옴)
+ A) 구현클래스는 인터페이스의 객체와 비슷하다 라고 받아들여, 상속이 아니라 인터페이스 변수에 구현객체를 대입하기때문

## 질문 3. Q) 인터페이스를 구현하는 구현 클래스의 자식 클래스는 구현 클래스에서 재정의된 추상클래스를 상속하나?
+ A) 재정의된 추상 클래스를 상속함

...
## Cf) Q) 자식클래스의 필드에서는 왜 this나 super로 부모클래스의 필드에 접근할 수 없는가? 
+ A) 우리가 어떤 클래스를 사용하려면 그 클래스를 정의 해서 다른 클래스에서 객체를 생성한 후 사용한다
+ 이때 new를 이용해 ()생성자를 호출하여 객체를 생성한다
+ 이 과정에서 생성자는 부모생성자를 호출해 부모필드를 초기화 하고 자식클래스를 초기화 한다
+ 자식 클래스에서 this나 super로 필드에서 값을 변경 해봤자 객체를 생성하면 모두 초기화 되기 때문에 오류가 발생하는것이다(할 필요가 없기 때문)

+ **생성자 내부나 메소드 내부에서는 this나 super없이 부모 클래스 필드의 변수들 사용가능**
